...
router.get('/public', authenticateToken, async (req, res) => {
  ...
  try {
    let query = `
      SELECT c.id, c.name, c.description, c.club_picture, c.max_members, c.created_at, c.url_slug,
             (SELECT COUNT(*) FROM club_members WHERE club_id = c.id AND is_active = true) as member_count,
             (SELECT COUNT(*) FROM cycles WHERE club_id = c.id) as total_cycles,
...
// Add this route to your existing clubs.js file at the end, before module.exports

// Get club statistics for a season
router.get('/:id/stats', authenticateToken, requireClubMembership, async (req, res) => {
  ...
  try {
    // Get season stats for all members
    const seasonStatsResult = await pool.query(
      `SELECT uss.*, u.username, u.display_name, u.profile_picture
       FROM user_season_stats uss
       JOIN users u ON uss.user_id = u.id
       JOIN club_members cm ON u.id = cm.user_id AND cm.club_id = uss.club_id
       WHERE uss.club_id = $1 AND uss.season_year = $2 AND cm.is_active = true
       ORDER BY uss.total_points DESC, uss.average_points DESC`,
      [clubId, seasonYear]
    );

    // Get detailed cycle results for accuracy and ranking stats
    const cycleResultsQuery = await pool.query(
      `SELECT cr.user_id, cr.final_rank, cr.guess_accuracy, cr.points_earned,
              u.username, u.display_name,
              c.cycle_number, c.completed_at
       FROM cycle_results cr
       JOIN cycles c ON cr.cycle_id = c.id
       JOIN users u ON cr.user_id = u.id
       JOIN club_members cm ON u.id = cm.user_id AND cm.club_id = c.club_id
       WHERE c.club_id = $1 AND c.season_year = $2 AND c.phase = 'idle' AND cm.is_active = true
       ORDER BY c.completed_at DESC, cr.final_rank ASC`,
      [clubId, seasonYear]
    );
    ...
    // Initialize with season stats
    seasonStatsResult.rows.forEach(stats => {
      userStats.set(stats.user_id, {
        user_id: stats.user_id,
        username: stats.username,
        display_name: stats.display_name,
        profile_picture: stats.profile_picture,
        cycles_participated: stats.cycles_participated,
        cycles_won: stats.cycles_won,
        total_points: parseFloat(stats.total_points),
        average_points: parseFloat(stats.average_points),
        first_place_finishes: 0,
        last_place_finishes: 0,
        average_guess_accuracy: 0,
        total_guess_accuracy: 0,
        accuracy_count: 0
      });
    });

    // Process cycle results for detailed stats
    const cyclesByUser = new Map();
    cycleResultsQuery.rows.forEach(result => {
      if (!cyclesByUser.has(result.user_id)) {
        cyclesByUser.set(result.user_id, []);
      }
      cyclesByUser.get(result.user_id).push(result);
    });

    // Calculate ranking stats
    cyclesByUser.forEach((cycles, userId) => {
      ...
      cycles.forEach(cycle => {
        // Count first and last place finishes
        if (cycle.final_rank === 1) firstPlaces++;
        
        // For last place, we need to know how many participants were in that cycle
        // We'll approximate by checking if this is the lowest rank for this cycle
        
        // Accuracy stats
        if (cycle.guess_accuracy !== null) {
          totalAccuracy += parseFloat(cycle.guess_accuracy);
          accuracyCount++;
        }
      });

      // Count last place finishes by checking if user had the highest rank in any cycle
      const lastPlaceQuery = cycleResultsQuery.rows.filter(r => 
        r.user_id === userId && cycles.some(c => c.cycle_number === r.cycle_number)
      );
      
      // Get unique cycles for this user and check ranks
      const userCycles = [...new Set(cycles.map(c => c.cycle_number))];
      userCycles.forEach(cycleNum => {
        const cycleParticipants = cycleResultsQuery.rows.filter(r => r.cycle_number === cycleNum);
        const maxRank = Math.max(...cycleParticipants.map(r => r.final_rank));
        const userRank = cycles.find(c => c.cycle_number === cycleNum)?.final_rank;
      ...
      user.first_place_finishes = firstPlaces;
      user.last_place_finishes = lastPlaces;
      user.average_guess_accuracy = accuracyCount > 0 ? totalAccuracy / accuracyCount : 0;
    });

    // Convert map to array and sort by total points
    const statsArray = Array.from(userStats.values()).sort((a, b) => b.total_points - a.total_points);

    // Get club season summary
    const seasonSummary = {
      total_members: statsArray.length,
      total_cycles: cycleResultsQuery.rows.length > 0 ? 
        Math.max(...cycleResultsQuery.rows.map(r => r.cycle_number)) : 0,
      season_year: parseInt(seasonYear),
      average_participation: statsArray.length > 0 ? 
        statsArray.reduce((sum, user) => sum + user.cycles_participated, 0) / statsArray.length : 0
    };

    res.json({
      season_summary: seasonSummary,
      user_stats: statsArray,
      season_year: parseInt(seasonYear)
    });
