...
// Get user profile by username - NEW ROUTE
router.get('/profile/by-username/:username', authenticateToken, async (req, res) => {
...
// Get user profile by ID (legacy route)
router.get('/profile/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;

  // If id is not a number, treat it as username for backward compatibility
  if (isNaN(id)) {
    return res.redirect(`/api/users/profile/by-username/${id}`);
  }
  ...
  try {
    ...
    const profile = {
      ...user,
      ...statsResult.rows[0],
      is_own_profile: user.id === req.user.id
    };
...
// Legacy route for old profile URLs
