...
const ClubPage = () => {
  ...
  const fetchClubData = async () => {
    try {
      ...
      // Fetch club details
      const clubResponse = await api.get(`/clubs/by-name/${clubname}`);
      setClub(clubResponse.data.club);
      
      // Fetch current cycle if exists
      if (clubResponse.data.club?.id) {
        try {
          const cycleResponse = await api.get(`/cycles/${clubResponse.data.club.id}/current`);
  ...
  const handleSubmitTheme = async () => {
    ...
    try {
      ...
      await api.post(`/clubs/${club.id}/themes`, { themeText: newTheme.trim() });
      setNewTheme('');
  ...
  const getImageUrl = (imagePath) => {
    ...
    // Use relative path for nginx proxy - nginx handles /uploads/ routing
    return imagePath;
  };
