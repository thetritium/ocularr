...
const ClubPage = () => {
  ...
  const handleStartCycle = async () => {
    ...
    try {
      ...
      const errorMsg = handleApiError(err, 'Failed to start cycle');
      setError(errorMsg);
    }
  };
  ...
  const handleProgressPhase = async () => {
    ...
    try {
      ...
      const errorMsg = handleApiError(err, 'Failed to progress phase');
      setError(errorMsg);
    }
  };

  const handleNominate = async () => {
    if (!selectedMovie || !currentCycle?.id) return;
    ...
    try {
      ...
      const errorMsg = handleApiError(err, 'Failed to nominate movie');
      setError(errorMsg);
    }
  };

  const handleAddTheme = async () => {
    if (!newTheme.trim() || !club?.id) return;
    
    try {
      setError('');
      await clubAPI.submitTheme(club.id, newTheme.trim());
      setShowThemeModal(false);
      setNewTheme('');
      await fetchThemes();
    } catch (err) {
      const errorMsg = handleApiError(err, 'Failed to add theme');
      setError(errorMsg);
    }
  };
  ...
  const handleLeaveClub = async () => {
    ...
    try {
      ...
      const errorMsg = handleApiError(err, 'Failed to leave club');
      setError(errorMsg);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="bg-gray-800 rounded-lg p-6 mb-6">
        <div className="flex items-start gap-6">
          ...
          <div 
            className={`w-32 h-32 bg-gray-700 rounded-lg flex items-center justify-center ${
              getClubPictureUrl(club.club_picture) ? 'hidden' : 'flex'
            }`}
          >
